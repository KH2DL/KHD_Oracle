...
/*
    ...
    - SELECT 쿼리의 실행 결과를 화면에 저장한 놀리적인 가상의 테이블
...
-- 1.2.1 Stored View(저장O) - 이름을 붙여서 저장함
    ...
    FROM EMPLOYEE;
-- 주의 : VEIW 만들때는 권한이 필요함, 권한명 : CREATE VIEW
-- System 계정으로 권한 부여
...
/*
    ...
    -- JOIN을 활용한 VIEW 생성가능
    1.4 목적
    - 원본 테이블이 아님 뷰를 통해 특정 데이터만 보이도록 함
...
-- 1.5.1 OR REPLACE
-- 수정하기
CREATE OR REPLACE VIEW V_EMPLOYEE(사번, 이름, 부서코드, 직급코드, 급여)
AS SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE, SALARY
FROM EMPLOYEE;

-- 1.5.2 FORCE / NOFORCE
-- 기존 테이블이 존재하지 않더라도 뷰를 생성하는 옵션
-- 1.5.3 WITH CHECK OPTION
-- WHERE 조건에서 사용한 컬럼의 값을 수정하지 못하게 하는 옵션
...
(SELECT EMP_NAME FROM EMPLOYEE WHERE EMP_ID = 매니저 ID);
...
-- 일요자일때는 1, 이용자일때는 2, ...
-- USER_NO 컬럼에 들어가는 데이터틑 누군가 기억하고 있어야 함.
-- 마지막 번호가 몇번이있는지, 몇씩 증가해서 들어가야하는지를 기억하고 있어야함.
-- 그런 역할을 하는 것이 바로 Sequence임.

-- 2.1 개요
-- - 순차작으로 정수 값을 자동으로 생성하는 객체, 자동 번호 발생기(채번기)의
--   역학을 함
