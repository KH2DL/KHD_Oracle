...
ROLLBACK;
-- TCL 명령어
-- 1. COMMIT : 트랜젝션 작업이 정상 완료되어 변경 내용을 영구히 저장(모든 savepoint 삭제)
-- 2. ROLLBACK : 트랜젝션 작업을 모두 취소하고 가장 최근 COMMIT 시점으로 이동
-- 3. SAVEPOINT <savepoint명> : 현재 트랜젝션 작업 시점에 이름을 지정함
-- 임시저장이라고 하며 하나의 트랜젝션에서 구역을 나눌 수 있음
...
ROLLBACK; -- 최종 커밋한 시점으로 이동, SAVAPOINT 삭제
