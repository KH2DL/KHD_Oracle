...
DECLARE
    VEMP EMPLOYEE%ROWTYPE; -- EMPLOYEE 의 모든 데이터를 하나로 가능
BEGIN
    SELECT *
    INTO VEMP
    FROM EMPLOYEE
...
DECLARE
    ...
    VDTTITLE  DEPARTMENT.DEPT_TITLE%TYPE;
BEGIN
    SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_TITLE
    INTO VEMPID, VENAME, VDCODE, VDTTITLE
    FROM EMPLOYEE
    LEFT OUTER JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID
    WHERE EMP_ID = '&EMP_ID';
    DBMS_OUTPUT.PUT_LINE('사원번호 : ' || VEMPID || '이름 : ' || VENAME || '부서코드 : ' || VDCODE || '부서명 : ' || VDTTITLE );
END;
/
...
-- 3. IF (조건식) THEN (실행문) ELSIF (조건식) THEN (실행문) EMD IF;
