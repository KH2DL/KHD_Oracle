...
-- 프로시져(PROCEDUAL), 함수(FUNTION), 트리거(TRIGGER), 동의어(SYSNONYM), 커서(CURSOR), 패키지(PACKAGE)...

-- 3. ROLE
-- 3.1 개요
-- - 사용자에게 여러 개의 권한을 한번에 부여할 수 있는 데이터베이스 객체
-- - 사용자에게 권한을 부여할 때 한개씩 부여하게 된다면 훤한 부여 및 회수의 관리가
--  분편하므로 사용하는 객체임
SELECT * FROM DBA_SYS_PRIVS 
ORDER BY 2 ASC; -- System계정으로 실행

--* DCL(Date Control Language) : GRANT, REVOKE
--  권한부여 및 회수는 권리자 세션에서 사용 가능
--  관리자 계정
...
-- - KEY와 VALUE형태로 생성이 되며 KET에는 인덱스로 만들 컬럼값, VALUE에는 행이 저장된
-- 주소값이 저장됨
-- - 장점 : 검색속도가 빨라지고 시스템에 걸리는 부하를 중여서 시스템 전체 성능을 향상시킬 수 있음.
-- - 단점 : 인데스를 위한 추가 저장 공간이 필요하고 인데스를 생성하는데 시간이 걸림
-- 데이터의 변경작업이 자주 일어나는 테이블에 INDEX 생성시 오히려 성능저하가 발생할 수 있음.
-- - 어떤 컬럼에 인덱스를 만들면 좋을까?
-- 데이터값이 중복된 것이 없는 고유한 데이터값을 가지는 컬럼에 만드는 것이 가장 좋음
-- 그리고 자주 사용되는 컬럼에 만들면 좋음! - 적대적이지는 않음
...
WHERE EMP_ID LIKE '2%';
SET TIMING OFF;
-- F10으로 싱행하여 오라클  PLAN 확인 가능, 튜닝시 사용됨.
