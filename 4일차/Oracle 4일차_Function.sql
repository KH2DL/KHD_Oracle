...
-- 1.1 단일행 함수의 종류
-- 1.1.1 숫자 처리 함수
--      - ABS, MOD, TRUNC, FLLOR, ROUND, CEIL
-- 1.1.2 문자 처리 함수
...
SELECT SYSDATE "입대일자", ADD_MONTHS(SYSDATE, 18) "제대일자"
...
-- CASE 함수
-- 사용법 : CASE WHEN 변하는값 = 조건값 THEN 참일때 값 WHEN 변하는 값 = 조건값 THEN 참일때 값 ELSE 모두 아닐때 값 EMD
...
, FLOOR(AVG(SALARY)) "급여 평균", COUNT(*) "인원수"
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
-- BONUS컬럼의 앖이 존재한다면 그 행을 1로 카운팅, 보너스를 지급받은 사원이 없는 부서도 있음을 확인
...
SELECT TO_CHAR(HIRE_DATE, 'YYYY"년"') "입사년도", COUNT(*) "인원수" FROM EMPLOYEE
WHERE JOB_CODE != 'J1'
...
SELECT 
DECODE(SUBSTR(EMP_NO,8,1), '1', '남', '2', '여', '3', '남', '4', '여', '무') "성별" 
, FLOOR(AVG(SALARY)) "평균급여", SUM(SALARY) "급여합계", COUNT(SALARY) "인원수"
FROM EMPLOYEE
...
-- 실습7
-- 부서내 성별 인원수를 구하시오
SELECT DEPT_CODE "부서명", DECODE(SUBSTR(EMP_NO,8,1), '1', '남', '2', '여', '3', '남', '4', '여', '무') "성별", COUNT(*) "인원수"
FROM EMPLOYEE
...
, SALARY, BONUS
, TO_CHAR(SALARY+(SALARY*NVL(BONUS, 0)), 99,999,999)"연봉(원)"
FROM EMPLOYEE;
