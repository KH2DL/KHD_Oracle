...
-- DROP TABLE,...
-- ALTER로 할 수 있었던 것
-- 컬럼추가, 컬럼삭제, 컬럼 수정(데이터타임, 컬럼명), 테이블명 수정 2가지

-- 제약조건 추가, 삭제, 수정
-- 제약조건 추가하면 이름이 생김, 이름을 알아야 삭제 가능
...
-- ex1) USER_FOREIGN_KEY 테이블에 REG_DATE DATE 컬럼을 추가해주세요
DESC USER_FOREIGN_KEY;
ALTER TABLE USER_FOREIGN_KEY
ADD REG_DATE DATE;
-- ex2) USER_FOREIGN_KEY 테이블에 USER_DATE DATE 컬럼을 삭제해주세요
ALTER TABLE USER_FOREIGN_KEY
DROP COLUMN USER_DATE;
-- ex3) USER_FOREIGN_KEY 테이블에 REG_DATE 컬럼의 자료형을 VARCHAR2(10)으로 바꿔주세요
ALTER TABLE USER_FOREIGN_KEY
MODIFY REG_DATE VARCHAR2(10);
-- ex4) USER_FOREIGN_KEY 테이블에 REG_DATE DATE 컬럼의 이름을 USER_DATE로 변경해주세요
ALTER TABLE USER_FOREIGN_KEY
RENAME COLUMN REG_DATE TO USER_DATE;
-- USER_DATE의 VARCHAR2(10)을 DATE로 바꿔주세요
ALTER TABLE USER_FOREIGN_KEY
MODIFY USER_DATE DATE;
-- ex5) USER_FOREIGN_KEY 테이블의 이름을 USER_TBL로 바꿔주세요 (2가지)
ALTER TABLE USER_FOREIGN_KEY
RENAME TO USER_TBL;
