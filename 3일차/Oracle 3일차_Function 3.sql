...
SELECT TRUNC(SYSDATE - HIRE_DATE) FROM EMPLOYEE;
...
SELECT ADD_MONTHS(SYSDATE, 2) FROM DUAL; -- 오늘로 부터 2개월 뒤를 출력해줌
SELECT MONTHS_BETWEEN(SYSDATE, '24/05/07') FROM DUAL;
-- ex1) EMPLOYEE 테이블에서 사원의 이름, 입사일, 입사 후 3개월이 된 날짜를 조회하시오.
SELECT EMP_NAME, HIRE_DATE, ADD_MONTHS(HIRE_DATE, 3) "입사 후 3개월" FROM EMPLOYEE;
DESC EMPLOYEE;
-- ex2) EMPLOYEE 테이블에서 사원의 이름, 입사일, 근무 개월수를 조회하시오
SELECT EMP_NAME, HIRE_DATE,
FLOOR((SYSDATE - HIRE_DATE)/30) "근무 개월수", 
FLOOR(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) "근무 개월수" -- 월마다 일수가 다름을 계산해서 더 정확함
FROM EMPLOYEE;
...
SELECT EMP_NAME, EXTRACT(YEAR FROM HIRE_DATE)||'년' "입사 년도"
