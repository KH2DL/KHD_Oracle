...
-- ResultSet은 특정한 기중에 의해 정렬될 수 있음
...
,SALARY*12+SALARY*NVL(BONUS,0) "총수력액"
,(SALARY*12+SALARY*NVL(BONUS,0)) - (SALARY*0.03) "실수령액"
FROM EMPLOYEE;

-- 2. EMPLOYEE 테이블에서 이름, 근무 일수를 출력해보시오.
-- (SYSDATE를 사용하면 현재 시간 출력)
SELECT EMP_NAME "이름", HIRE_DATE "입사일", SYSDATE "오늘"
, ROUND(SYSDATE - HIRE_DATE) "근무일수"
FROM EMPLOYEE;

-- 3. EMPLOYEE 테이블에서 20년 이상 근속자의 이름,월급,보너스율 출력하시오
SELECT EMP_NAME, SALARY, NVL(BONUS,0)
FROM EMPLOYEE WHERE (SYSDATE - HIRE_DATE) >= 7300;
...
-- DEPT_CODE가 D9 또는 D6이고 고용일이 90/01/01 ~ 01/12/01이면서, 
...
SELECT EMP_NAME "이름", EMP_NO "주민등록번호", SALARY "급여", HIRE_DATE "입사일" 
FROM EMPLOYEE
WHERE (SYSDATE - HIRE_DATE) >= 1825 AND ROUND(SYSDATE - HIRE_DATE) <= 3650;
-- 문제2.
-- 재직중이 아닌 직원의 이름,부서코드, 고용일, 근무기간, 퇴직일을 검색하여라 
--(퇴사 여부 : ENT_YN)
SELECT EMP_NAME "이름", JOB_CODE "부서코드",
...
SELECT EMP_NAME "이름", (SALARY*1.5) "인상된 급여", 
FLOOR((SYSDATE - HIRE_DATE)/365) "근속년수" -- FLOOR 소수점 내림
FROM EMPLOYEE
WHERE FLOOR((SYSDATE - HIRE_DATE)/365) >= 10
-- ORDER BY 근속년수 ASC; -- 별칭으로 가능
ORDER BY 3 ASC; -- 컬럼 위치 번호로도 가능
-- 문제4.
-- 입사일이 99/01/01 ~ 10/01/01 인 사람 중에서 급여가 2000000 원 이하인 사람의
-- 이름,주민번호,이메일,폰번호,급여를 검색 하시오
SELECT EMP_NAME "이름", EMP_NO "주민번호", EMAIL "이메일", PHONE "폰번호",
SALARY "급여"
FROM EMPLOYEE
WHERE HIRE_DATE BETWEEN '99/0101' AND '10/01/01'
AND SALARY <= 2000000;
-- 문제5.
