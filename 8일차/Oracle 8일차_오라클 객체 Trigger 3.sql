...
FOR EACH ROW
BEGIN
    IF(:OLD.SALARY != :NEW.SALARY) -- EMPLOYEE테이블에 다른 값이 UPDATE가 되면 실행되는 것을 방지
    THEN
...
-- 의사레코드 OLD, NEW
-- FOR EACH ROW를 사용
-- 1. INSERT : OLD -> NULL, NEW만 있음
-- 2. UPDATE : OLD, NEW
-- 3. DELETE : OLD만 있음, NEW -> NULL
...
--    CONSTRAINT FK_PRODUCT_ID FOREIGN KEY(PCODE) REFERENCES PRODUCT(PCODE)
);
