...
-- 1. TOP-N 분석
-- 특정 컬럼에서 가장 큰 N개의 값 또는 가장 작은 N개의 값을 구해야할 경우 사용
...
-- 1.1 ROWNUM, ROWID
-- 테이블을 새성하면 자동으로 만들어짐
-- ROWID : 테이블의 특정 레코드를 랜덤하게 접근하기 위한 노리적인 주소값
...
SELECT *
FROM (SELECT DEPT_CODE, DEPT_TITLE, FLOOR(AVG(SALARY)) "AVG_SAL"
FROM EMPLOYEE
...
-- 2. WITH
-- 서브쿼리에 이름을 붙여주고 인라인뷰로 사용시 서브쿼리의 이름을 FROM절에 
--  기술할 수 있음. 같은 서브쿼리가 여러번 사용될 경우 중복 작성을 필할 수 있고
--  실행속도도 빨라지는 장점이 있음.
-- WITH 서브쿼리명 AS(서브쿼리)
-- SELECT * FROM (서브쿼리명)
...
-- @실습문제2
-- 부서별 급여평균 TOP3 부서의 부서코드와 부서명, 평균급여를 출력하세요.
WITH TOP3_DCODE_AVGSAL AS
(SELECT DEPT_CODE, DEPT_TITLE, FLOOR(AVG(SALARY)) "AVG_SAL"
FROM EMPLOYEE
JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID
GROUP BY DEPT_CODE, DEPT_TITLE
ORDER BY AVG_SAL DESC)
SELECT * FROM TOP3_DCODE_AVGSAL
WHERE ROWNUM <= 3;

-- 3. 계층형 쿼리
-- JOIN을 통해 수평적으로 기준컬럼을 연결시킨 것과는 달리 기준컴럼으로 가지고
-- 수직적인 관계를 만듬
-- 예시) 조직도, 메뉴, 답변형 게시판 등 프랙탈 구조의 표현에 적합함
-- 오라클에서 사용되는 구문
-- 1. START WITH : 부모행(루트)를 지정
-- 2. CONNECT BY : 부모-자식관계를 지정
-- 3. PRIOR : START WITH절에서 제시한 부모형의 기준컬럼을 지정함.
-- 4. LEVEL : 의사컬럼(PSEUDO COLUMN), 계층정보를 나타내는 가상컬럼이며
-- SELECT, WHERE, ORDER BY에서 사용가능

-- 예제) 1명이라도 직원을 관리하는 매니저의 정보(사번, 이름, 매니저 아이디)를 출력하시오
...
-- PRIOR 다음에 나오는 컬럼은 START WITH에서 사용된 컬럼이다
...
-- 특정 컬럼 기준으로 랭킹을 부여함, 중복 군위 다음은 해당 갯수만큼 건너뛰고 반환함.
-- 예제) 회사의 연봉 순위를 출력하시오.
...
SELECT
    EMP_NAME "이름"
    , HIRE_DATE "입사일"
    , RANK() OVER(ORDER BY HIRE_DATE ASC) "입사일 순서"
FROM EMPLOYEE
...
-- ROWNUM과 RANK의 순위가 같다는 가정하에 사용 - 예제는 두 값이 다름 -> 급여순위가 틀림
SELECT EMP_NAME "이름", SALARY "급여", RANK() OVER(ORDER BY SALARY DESC) "급여 순위"
FROM EMPLOYEE
WHERE ROWNUM <= 10;

SELECT * FROM
(SELECT EMP_NAME "이름", SALARY "급여", RANK() OVER(ORDER BY SALARY DESC) "급여순위"
FROM EMPLOYEE)
